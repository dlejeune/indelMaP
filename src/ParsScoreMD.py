...
def ParsLeaf(leaf, alphabet):
    ...
    for i in range(len(leaf.sequence)):
        if alphabet == 'Protein':
            ...
            elif leaf.sequence[i] == '-':
                pars_sets.append(set([ 'A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 
                                      'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 
                                      'T', 'V', 'W', 'Y' ]))
...
def ParsScoreMD(tree_file, msa_file, out_file, alphabet, ancestor_reconstruction=True):
    '''
